...
def find_best_n_discords(profile, index, n):
    ...
    :param profile: TSA array with the matrix profile containing the minimum distance of each subsequence.
    ...
    :return: TSA arrays with the discord distances, the discord indices and the subsequence indices.
    """
    b = ctypes.c_void_p(0)
    c = ctypes.c_void_p(0)
    d = ctypes.c_void_p(0)
...
def find_best_n_motifs(profile, index, n):
    ...
    :return: TSA arrays with the motif distances, the motif indices and the subsequence indices.
    """
    b = ctypes.c_void_p(0)
    c = ctypes.c_void_p(0)
    d = ctypes.c_void_p(0)
...
def stomp(first_time_series, second_time_series, subsequence_length):
    ...
    :param first_time_series: TSA array with the first time series.
    :param second_time_series: TSA array with the second time series.
    :param subsequence_length: Length of the subsequence.
    :return: TSA arrays with the profile and index.
    """
...
def stomp_self_join(time_series, subsequence_length):
    ...
    :param time_series: The query and reference time series in TSA array format.
